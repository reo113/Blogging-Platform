...
export async function loader({ params }) {
  const jobResponse = await fetch(`http://localhost:3000/blogs/${params.jobId}`);
...
export async function action({ request, params }) {
  ...
  const response = await fetch(`http://localhost:3000/blogs/${params.jobId}`, { 
    method: "PATCH", 
    headers: {
  ...
  return redirect(`/blogs/${params.jobId}`)
}
